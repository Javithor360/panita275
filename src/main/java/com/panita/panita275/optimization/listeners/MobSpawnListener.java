...
public class MobSpawnListener implements Listener {
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        if (Panitacraft.getConfigManager().getBoolean("optimization.mobSpawning.reduceWolfs", false)) {
            ...
            if (entity instanceof Wolf wolf) {
                if (entity.getEntitySpawnReason() == CreatureSpawnEvent.SpawnReason.NATURAL && wolf.isAware() && !wolf.isTamed()
                    ...
                    if (Panitacraft.getConfigManager().getBoolean("optimization.debug", false)) {
